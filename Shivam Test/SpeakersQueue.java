...
public class SpeakersQueue {
  ...
  public boolean isEmpty() {
    return size == 0; // TODO CHANGE THIS: included for compiler
  }
  ...
  public void enqueue(String newName) {
    ...
    if (this.size == names.length) {
		...
		names[tailIndex % names.length] = newName;
		tailIndex++;
		size++;
	}
  ...
  public String toString() {
    ...
    if (headIndex != -1) {
		for (int start = headIndex; start < tailIndex; start++) {
			repr += names[(start % names.length)];
			repr += " ";
		}
	}
