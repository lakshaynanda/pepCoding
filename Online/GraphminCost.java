...
public class GraphminCost {
	...
	static void minCostPath(ArrayList<ArrayList<edge>> graph, int s, int d,boolean[] visited, int cost) {
		...
		for(int e=0;e<graph.get(s).size();e++)
		{
		    ...
		    if(!visited[ce.v2])
		    {
		        minCostPath(graph,ce.v2,d,visited,cost+ce.wt);
...
// 		int cost=scn.nextInt();
		...
		minCostPath(graph, a, b, visited,0);
